...
if __name__ == "__main__":
    ...
    # for Ticker in ['^IXIC']:
        ...
        asyncio.run(send_telegram(f'Mean Absolute Error: %  \033[1m{mae*100:.2f}'))
        asyncio.run(send_telegram(f'Mean Absolute Scaled Error: % \033[1m{mase_value*100:.2f}'))
        asyncio.run(send_telegram(f'Symmetric Mean Absolute Percentage Error: % \033[1m{smape_value*100:.2f}'))
        asyncio.run(send_telegram(f'Root Mean Square Error: % \033[1m{rmse*100:.2f}'))
